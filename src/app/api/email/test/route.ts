...
import { emailService, sendWelcomeEmail, sendCourtReminder } from '@/lib/email'
...
This is an automated test email from the Law Firm Platform.`
      ...
      default:
        return NextResponse.json(
          { error: 'Invalid test type. Use: basic, welcome, or court_reminder' },
          { status: 400 }
        )
    }
...
export const GET = withRole(ROLES.SUPER_ADMIN, async () => {
  try {
    ...
    const configuration = {
      ...
      fromEmail: process.env.FROM_EMAIL || 'noreply@lawfirm.example.com',
